...
import {
  Receipt, Plus, Edit2, Trash2, Search, X, Save,
  ChevronLeft, ChevronRight, Upload, ExternalLink, Building2
} from 'lucide-react';

interface MembershipExpense {
  id: string;
  description: string;
  amount: number;
...
const emptyForm: Partial<MembershipExpense> = {
  description: '',
  amount: 0,
  beneficiary: '',
...
export const MembershipExpensesTab = () => {
  const { language } = useLanguage();
  const isRTL = language === 'ar';

  const [records, setRecords] = useState<MembershipExpense[]>([]);
  ...
  const [editRecord, setEditRecord] = useState<Partial<MembershipExpense>>(emptyForm);
  const [isEditing, setIsEditing] = useState(false);

  useEffect(() => { fetchRecords(); }, [page, searchDesc, filterDateFrom, filterDateTo]);
  ...
  const handleUploadInvoice = async (file: File) => {
    ...
    try {
      ...
      toast.error(isRTL ? 'خطأ في الرفع — تأكد من bucket financial-docs' : 'Upload error');
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isEditing && editRecord.id) {
        const { error } = await supabase.from('membership_expenses').update(payload).eq('id', editRecord.id);
        if (error) throw error;
        toast.success(isRTL ? '✓ تم التعديل' : '✓ Updated');
      } else {
        const { error } = await supabase.from('membership_expenses').insert(payload);
        if (error) throw error;
  ...
  const handleDelete = async (id: string) => {
    ...
    const { error } = await supabase.from('membership_expenses').delete().eq('id', id);
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="bg-gradient-to-r from-orange-500 to-amber-500 rounded-2xl p-5 text-white relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-xl font-black">{isRTL ? 'مصاريف الانتساب' : 'Membership Expenses'}</h3>
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-white text-orange-600 rounded-xl font-bold text-sm hover:bg-orange-50 transition-colors shadow-sm"
          >
        ...
        <div className="flex flex-wrap gap-3">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input type="text" placeholder={isRTL ? 'بحث بالوصف...' : 'Search description...'} value={searchDesc}
              ...
              className={`w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl py-2.5 text-sm outline-none focus:ring-2 focus:ring-orange-500 ${isRTL ? 'pr-9 pl-4' : 'pl-9 pr-4'}`} />
          </div>
          <input type="date" value={filterDateFrom} onChange={e => { setFilterDateFrom(e.target.value); setPage(0); }}
            className="bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-orange-500" />
            ...
            <table className="w-full">
              <thead className="bg-slate-50 dark:bg-slate-800/60 border-b border-slate-100 dark:border-slate-800">
                <tr>
                  ...
                  <th className="px-5 py-3.5 text-start text-xs font-bold text-slate-400 uppercase tracking-wider">{isRTL ? 'الجهة المستفيدة' : 'Beneficiary'}</th>
                ...
                {records.length === 0 ? (
                  <tr><td colSpan={6} className="py-16 text-center">
                    <div className="flex flex-col items-center text-slate-400">
                      ...
                      <p className="font-medium">{isRTL ? 'لا توجد مصاريف' : 'No expenses yet'}</p>
                    </div>
                  </td></tr>
                ) : records.map(rec => (
                  <tr key={rec.id} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/40 transition-colors">
                    ...
                    <td className="px-5 py-4">
                      <span className="font-black text-orange-600 dark:text-orange-400">{Number(rec.amount).toLocaleString()} MRU</span>
                    </td>
                    <td className="px-5 py-4">
                      {rec.beneficiary ? (
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 rounded-lg text-xs font-bold">
                          <Building2 className="w-3 h-3" />{rec.beneficiary}
                        </span>
          ...
          <div className="flex items-center justify-between px-5 py-3.5 border-t border-slate-100 dark:border-slate-800">
            ...
            <div className="flex gap-2">
              <button onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0}
                className="p-2 rounded-xl bg-slate-50 dark:bg-slate-800 text-slate-500 hover:bg-slate-100 disabled:opacity-40">
        ...
        {showModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <motion.div
              ...
              dir={isRTL ? 'rtl' : 'ltr'}
            >
              <div className="bg-gradient-to-r from-orange-600 to-amber-600 p-6 text-white shrink-0">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-black">
                    {isEditing ? (isRTL ? 'تعديل المصروف' : 'Edit Expense') : (isRTL ? 'إضافة مصروف انتساب' : 'Add Membership Expense')}
                  </h3>
              ...
              <div className="p-6 space-y-4 overflow-y-auto">
                <div>
                  ...
                  <input type="text" value={editRecord.description || ''} onChange={e => setEditRecord(p => ({ ...p, description: e.target.value }))}
                    className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-orange-500" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">{isRTL ? 'الجهة المستفيدة' : 'Beneficiary'}</label>
                  ...
                  {editRecord.invoice_url ? (
                    <div className="flex items-center gap-3 p-3 bg-teal-50 dark:bg-teal-900/20 rounded-xl">
                      <a href={editRecord.invoice_url} target="_blank" rel="noreferrer"
                        ...
                        <ExternalLink className="w-4 h-4" />{isRTL ? 'عرض الفاتورة' : 'View Invoice'}
                      </a>
                      <button onClick={() => setEditRecord(p => ({ ...p, invoice_url: null }))}
                        className="p-1 text-slate-400 hover:text-red-500 transition-colors"><X className="w-4 h-4" /></button>
                    </div>
                  ) : (
                    <label className="flex flex-col items-center gap-2 p-4 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl cursor-pointer hover:border-orange-400 hover:bg-orange-50/50 dark:hover:bg-orange-900/10 transition-colors">
                      {uploading ? <div className="w-6 h-6 border-2 border-orange-500/30 border-t-orange-500 rounded-full animate-spin" /> : <Upload className="w-6 h-6 text-slate-400" />}
                ...
                <div>
                  ...
                  <textarea rows={2} value={editRecord.notes || ''} onChange={e => setEditRecord(p => ({ ...p, notes: e.target.value }))}
                    className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-orange-500 resize-none" />
                </div>
              </div>
              ...
              <div className="shrink-0 px-6 py-4 border-t border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 flex gap-3">
                ...
                <button onClick={handleSave} disabled={saving}
                  className="flex-1 py-3 rounded-xl bg-orange-500 hover:bg-orange-600 text-white font-bold flex items-center justify-center gap-2 transition-colors disabled:opacity-60">
